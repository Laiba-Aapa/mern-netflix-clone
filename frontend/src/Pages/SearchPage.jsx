...
const SearchPage = () => {
  ...
  const handleTabClick = (type) => {
    setActiveTab(type);
    type === "movie" ? setContentType("movie") : setContentType("tv");
    setResults([]); // gonna remove all data if we change contentType
    setSearchTerm("");
  };
  ...
  const handleContentClick = async (content) => {
    try {
      ...
      // Send the selected movie's data (id) to the server to store in the database
      await apiRequest.post("/search/save", {
        content: content,
        searchType: activeTab,
      });
    } catch (error) {
      toast.error("Error saving the movie:", error.message);
    }
  };
